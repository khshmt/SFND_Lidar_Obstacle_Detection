...
{
    auto startTime = std::chrono::steady_clock::now(); //register the time of the beggining of the segmentation process
    ...
    seg.segment(*inliers, *coff); //the output is the indcies of the points of the grounf plane
...
{
    auto startTime = std::chrono::steady_clock::now(); //register the time of the beggining of the clustering process

    std::vector<typename pcl::PointCloud<PointT>::Ptr> clusters;

    /*clustering the cloud except the ground using KDtree method to oragnise the cloud and the euclidean clustering method*/
    ...
    ec.setMinClusterSize(minSize); //minimum number of points can be considered as a cluster
    ec.setMaxClusterSize(maxSize); //maximum number of points can be considered as a cluster
    ec.setSearchMethod(tree); //searchmethod as tree method
    ec.setInputCloud(cloud);
    ec.extract(cluster_indices); //the output is vector of clusters indicies
    ...
    auto endTime = std::chrono::steady_clock::now(); //register of the end of the clustering process
    auto elapsedTime = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
    
    std::cout << "clustering took " << elapsedTime.count() << " milliseconds and found " << clusters.size() << " clusters" << std::endl;

    return clusters; //the returned value is vector of pointer to the point cloud of each cluster
}
