...
std::unordered_set<int> Ransac2D(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, int maxIterations, float distanceTol)
{
	...
	for(int i=0; i < maxIterations; i++)
	{
		...
		//SAMPLING
		while(inliers.size()<2) //we sample 3 pints in case of 3D segmentaion and s points in 2D segmentation
		{
		...
		//COMPUTE A MODEL
		float slope = (point1.y - point2.y) / (point1.x -point2.x); //slope = (y2-y1)/(x2-x1)
		float y_intercept = point1.y - slope*point1.x; //c = y -ax
		...
		for(int index=0; index < cloud->points.size(); index++) 
		{
			...
			float d = std::fabs(slope*(cloud->points[index].x) - cloud->points[index].y + y_intercept) / std::sqrt(pow(-1,2) + pow(slope, 2));
...
std::unordered_set<int> Ransac3D(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, int maxIterations, float distanceTol)
{
	...
	for(int i=0; i < maxIterations; i++)
	{
		...
		for(int index=0; index < cloud->points.size(); index++) 
		{
			...
			float d = std::abs(A*cloud->points[index].x + B*cloud->points[index].y + C*cloud->points[index].z) / std::sqrt(A*A + B*B + C*C);
